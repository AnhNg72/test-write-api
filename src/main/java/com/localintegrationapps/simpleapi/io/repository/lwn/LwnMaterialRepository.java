...
@Repository
public class LwnMaterialRepository {
    private Logger log = LoggerFactory.getLogger(LwnMaterialRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO LWN.MATERIAL AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?,?)) AS S \n" +
            ...
            " WHEN MATCHED \n" +
            " AND ( \n" +
            " COALESCE(T.aktives_teil,'') <> COALESCE(S.aktives_teil,'') \n" +
    ...
    public LwnMaterialRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<LwnMaterialEntity> articles) {
