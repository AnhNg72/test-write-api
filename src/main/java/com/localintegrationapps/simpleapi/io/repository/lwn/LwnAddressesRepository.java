...
@Repository
public class LwnAddressesRepository  {
    private Logger log = LoggerFactory.getLogger(LwnAddressesRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO LWN.ADDRESSES AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public LwnAddressesRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<LwnAddressEntity> addresses) {
        jdbc.batchUpdate(UPSERT_SQL, addresses, 1, (ps, it) -> {
