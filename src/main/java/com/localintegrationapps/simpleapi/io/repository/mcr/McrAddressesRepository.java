...
@Repository
public class McrAddressesRepository {
    private Logger log = LoggerFactory.getLogger(McrAddressesRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO MCR.ADDRESSES AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public McrAddressesRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<McrAddressEntity> addresses) {
