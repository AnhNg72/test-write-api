...
@Repository
public class LwnDocumentsRepository {
    private Logger log = LoggerFactory.getLogger(LwnDocumentsRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO LWN.DOCUMENTS AS T USING (VALUES (?,?,?,?,?,?,?)) AS S \n" +
    ...
    public LwnDocumentsRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<LwnDocumentEntity> documents) {
