...
@Repository
public class LwnOrdersRepository {
    private Logger log = LoggerFactory.getLogger(LwnOrdersRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO LWN.ORDERS AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public LwnOrdersRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<LwnOrderEntity> orders) {
