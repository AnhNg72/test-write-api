...
@Repository
public class McrDocumentsRepository {
    private Logger log = LoggerFactory.getLogger(McrDocumentsRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO MCR.DOCUMENTS AS T USING (VALUES (?,?,?,?,?,?,?)) AS S \n" +
    ...
    public McrDocumentsRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<McrDocumentEntity> documents) {
