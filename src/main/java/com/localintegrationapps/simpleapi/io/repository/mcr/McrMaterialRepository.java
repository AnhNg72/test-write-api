...
@Repository
public class McrMaterialRepository {
    private Logger log = LoggerFactory.getLogger(McrMaterialRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO MCR.MATERIAL AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public McrMaterialRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<McrMaterialEntity> articles) {
