...
@Repository
public class McrComponentsRepository {
    private Logger log = LoggerFactory.getLogger(McrComponentsRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO MCR.COMPONENTS AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public McrComponentsRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<McrComponentEntity> components) {
