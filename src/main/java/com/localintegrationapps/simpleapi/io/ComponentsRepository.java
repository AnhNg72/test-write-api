...
public class ComponentsRepository {
    private Logger log = LoggerFactory.getLogger(ComponentsRepository.class);

    public static final String UPSERT_SQL = "MERGE INTO COMPONENTS AS T USING (VALUES (?,?,?,?,?,?,?,?,?,?)) AS S \n" +
    ...
    public ComponentsRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void upsertBatch(List<ComponentEntity> components) {
